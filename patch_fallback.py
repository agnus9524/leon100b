...
fallback = """      const result = await model.generateContent(prompt);
...
if old in content:
    content = content.replace(old, fallback)
    with open('server.ts', 'w') as f:
        f.write(content)
    print("Patched fallback successfully.")
else:
    print("Fallback string not found!")
