...
export const ScalperRecommendationsModal: React.FC<ScalperRecommendationsModalProps> = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-[9999] flex items-center justify-center p-2.5 sm:p-4 md:p-6 overflow-y-auto">
        <motion.div
          initial={{ opacity: 0, scale: 0.94, y: 15 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.94, y: 15 }}
          className="bg-slate-950 border border-emerald-500/40 rounded-3xl max-w-5xl w-full flex flex-col max-h-[92vh] shadow-[0_0_60px_rgba(16,185,129,0.25)] relative text-white overflow-hidden"
        >
          ...
          {/* Filter Bar */}
          <div className="px-4 sm:px-5 py-2.5 bg-slate-900/40 border-b border-slate-800/60 flex items-center justify-between gap-2 flex-wrap shrink-0">
            <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
              {[
                { id: 'ALL', label: '전체 TOP 10', icon: Trophy, count: recommendations.length },
                { id: 'VOLUME_SURGE', label: '거래량 폭증형', icon: Flame, count: recommendations.filter(r => r.category === 'VOLUME_SURGE').length },
                { id: 'MOMENTUM_BREAKOUT', label: '모멘텀 돌파형', icon: Zap, count: recommendations.filter(r => r.category === 'MOMENTUM_BREAKOUT').length },
                { id: 'SUPPORT_REBOUND', label: '눌림목 반등형', icon: Target, count: recommendations.filter(r => r.category === 'SUPPORT_REBOUND').length }
              ].map(tab => {
                const Icon = tab.icon;
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold font-sans flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap ${
                      active 
                  ...
                  >
                    ...
                    <span>{tab.label}</span>
                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono font-black ${active ? 'bg-emerald-500/30 text-emerald-200' : 'bg-slate-700/50 text-slate-400'}`}>
                      {tab.count}
                    </span>
                  </button>
                );
            ...
            {top3.length > 0 && (
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 shadow-[0_0_15px_rgba(16,185,129,0.3)] active:scale-95"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>TOP 3 종목 탭 일괄 등록</span>
              </button>
            )}
          </div>
