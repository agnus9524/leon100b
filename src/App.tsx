...
setGapInventory(nextInv);
  ...
  return;
    ...
    if (sameMarketNow.length >= MAX_INVENTORY_PER_MARKET) {
      let inactiveIdx = -1;
    ...
    try {
      ...
      if (priceData && priceData.current > 0) {
        ...
        setScalperInventory(prev => {
          ...
          if (!cur.pullback && sensor.isPullback) sensorTransitions.push('PULLBACK(눌림목) 감지');
          if (!cur.breakout && sensor.isBreakout) sensorTransitions.push('BREAKOUT(돌파) 감지');
          if (!cur.vwap && sensor.isVwapSupport) sensorTransitions.push('VWAP SUPPORT 감지');
          if (!cur.cvd && sensor.isVolumeProfile) sensorTransitions.push('CVD(거래량 프로파일) 감지');
          if (sensorTransitions.length > 0) {
            setTimeout(() => {
              sensorTransitions.forEach(msg => addLog(symbol, '매수', updatedStock.price || 0, 0, `[전략센서] ${msg}`));
            }, 0);
          }
...
useEffect(() => {
    ...
    try {
      for (const order of currentPending) {
        ...
        const isUSStock =
          ...
          /^[A-Za-z]/.test(currentStock.symbol) ||
          marketType === 'US';
        ...
        const tickDiff =
          tickSize > 0
            ? (currentPrice - orderPrice) / tickSize
            : 0;
        ...
        const dropPercent =
          ((orderPrice - currentPrice) / orderPrice) * 100;
        ...
        const isDropCancel =
          tickDiff <= -3 ||
          dropPercent >= autoCancelThreshold;
        ...
        if (isDropCancel || isRiseCancel) {
          ...
          const tickStr =
            Math.abs(Math.round(tickDiff)) > 0
              ? `${Math.abs(Math.round(tickDiff))}틱`
              : '';
          ...
          // --------------------------------------------------
          // 아직 KIS 주문번호가 없는 임시 주문
          // --------------------------------------------------

          if (
            !order.id ||
            order.id.startsWith('SLOT-')
          ) {
            console.log(
              `[Auto-Cancel] In-flight order cleared: ${formatCurrency(orderPrice)} (${cancelReason})`
            );
          ...
          if (
            ...
            try {
              ...
              setBotStatus(
                `[KIS API] 주문번호 ${order.id} 자동취소 요청 중...`
              );
              ...
              if (
                ...
                addLog(
                  ...
                  orderPrice,
                  order.quantity,
                  `[KIS 자동취소] ${cancelReason}`
                );

                showNotification(
                  `${currentStock.name} KIS 매수 미체결 자동 취소 완료`,
                  "info"
                );

                setBotStatus(
                  `[KIS 취소완료] ${formatCurrency(orderPrice)} 미체결 주문 취소`
                );
              ...
              try {
                ...
                const status =
                  await kisService.checkOrderExecution(
                    order.id
                  );
                ...
                if (newlyFilledQty > 0) {
                  ...
                  addLog(
                    ...
                    filledPrice,
                    newlyFilledQty,
                    `[취소 중 체결] 신규 ${newlyFilledQty}주 / 누적 ${totalFilledQty}주 / 실제체결가 ${formatCurrency(filledPrice)}`
                  );

                  showNotification(
                    `${currentStock.name} ${newlyFilledQty}주 체결 확인`,
                    "success"
                  );
              ...
              setBotStatus(
                `[자동취소 완료] ${formatCurrency(orderPrice)} 주문 정리됨`
              );
          ...
          try {
            ...
            if (newlyFilledQty > 0) {
              ...
              addLog(
                ...
                filledPrice,
                newlyFilledQty,
                `[실제체결] 신규 ${newlyFilledQty}주 / 누적 ${totalFilledQty}주 / 실제체결가 ${formatCurrency(filledPrice)}`
              );

              showNotification(
                `${currentStock.name} KIS 매수 ${newlyFilledQty}주 체결`,
                "success"
              );

              setBotStatus(
                `[체결] ${currentStock.name} 실제체결가 ${formatCurrency(filledPrice)} × ${newlyFilledQty}주`
              );
  ...
  const addLog = (
    ...
    setTradeLogs(prev => [newLog, ...prev].slice(0, 300));
        ...
        <aside className="border-t lg:border-t-0 lg:border-l border-white/5 bg-black/30 flex flex-col p-4 sm:p-5 lg:p-6 gap-4 sm:gap-6 overflow-hidden lg:col-start-2 lg:row-start-1 lg:row-span-2">
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-5 flex flex-col h-[640px] shrink-0 overflow-hidden shadow-2xl">
              ...
              <div className="flex items-center gap-1.5 flex-wrap mb-3 shrink-0">
                <button
                  type="button"
                  onClick={() => setLogFilterSymbol('ALL')}
                  className={cn(
                ...
                {scalperTabs.map(tab => (
                  <button
                    key={tab.symbol}
                    type="button"
                    onClick={() => setLogFilterSymbol(tab.symbol)}
                    className={cn(
                      "px-2.5 py-1 rounded-lg text-[11px] font-bold border transition-all truncate max-w-[100px]",
                      logFilterSymbol === tab.symbol
                        ? "bg-sleek-blue/25 border-sleek-blue/60 text-sleek-blue"
                        : "bg-black/30 border-white/10 text-slate-400 hover:text-white hover:border-white/30"
                    )}
                    title={`${tab.name} (${tab.symbol})`}
                  >
                    {tab.name}
                  </button>
                ))}
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1.5 pr-1">
                {(() => {
                  ...
                  if (filteredLogs.length === 0) {
                    return (
                      <div className="text-[11px] text-gray-500 text-center py-6 font-mono">
                        {logFilterSymbol === 'ALL' ? '아직 이벤트가 없습니다. START AI SCALPER를 눌러 감시를 시작하세요.' : `[${logFilterSymbol}] 종목의 이벤트가 없습니다.`}
                      </div>
                    );
                  }
                  // 스크롤 가능하므로 조금 더 넉넉히 렌더링해도 괜찮다 (최근 30건까지)
                  return filteredLogs.slice(0, 300).map((log, idx) => {
