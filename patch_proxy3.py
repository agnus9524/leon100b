...
pattern = r"    try \{\s*const axiosConfig.*?\}\s*catch \(error: any\) \{.*?return res\.status\(200\)\.json\(\{ \s*rt_cd: '-1'.*?\}\);\s*\}\s*\}"
...
content = re.sub(pattern, new_proxy, content, flags=re.DOTALL)

with open('server.ts', 'w') as f:
    f.write(content)
print("Patched KIS proxy via regex")
